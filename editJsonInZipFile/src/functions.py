...
from src.variables import directory
...
def read_zip_file(fullZipPath):
    ...
    with ZipFile(fullZipPath) as zfile:
...
def update_json_file(jsonFileName, key, value):
    ...
    with open(jsonFileName, "r+") as jsonFile:
        ...
        dump(data, jsonFile, sort_keys=True, indent=2)
        jsonFile.truncate()
...
def prepareZipFile(directory, newFileToZip):
    ...
    with ZipFile(newFileToZip,'w') as zip:
        # writing each file one by one
        for file in file_paths:
            zip.write(file)
    print('All files zipped successfully!')

def run(args,dir_path):
...
def main(ArgumentParser, dir_path):
    """ Main function to get arguments from console """
    parser = ArgumentParser(description="Extract and edit JSON file from ZIP. Then create new ZIP from edited file.")
